            ...
            switch (property) {
              case "type": {
                ...
                if (database === DB.MYSQL || database === DB.MARIADB) {
                  up.push(
                    `ALTER TABLE ${q(name)} MODIFY COLUMN ${q(columnName)} ${assertSafeType(change.to)};`,
                  );
                  down.push(
                    `ALTER TABLE ${q(name)} MODIFY COLUMN ${q(columnName)} ${assertSafeType(change.from)};`,
                  );
              ...
              case "size": {
                ...
                if (database === DB.MYSQL || database === DB.MARIADB) {
                  up.push(
                    `ALTER TABLE ${q(name)} MODIFY COLUMN ${q(columnName)} ${columnType}(${assertSafeSize(change.to)});`,
                  );
                  down.push(
                    `ALTER TABLE ${q(name)} MODIFY COLUMN ${q(columnName)} ${columnType}(${assertSafeSize(change.from)});`,
                  );
                } else if (database === DB.ORACLESQL) {
                  up.push(
