...
function toTable(table, db) {
  ...
  if (db === DB.POSTGRES) {
    fieldDefs = table.fields
      .map(
        (f) =>
          `${exportFieldComment(f.comment)}\t${q(f.name)} ${f.type}${f.size ? `(${f.size})` : ""}${f.isArray ? " ARRAY" : ""}${f.notNull ? " NOT NULL" : ""}${f.unique ? " UNIQUE" : ""}${f.increment ? " GENERATED BY DEFAULT AS IDENTITY" : ""}${f.default?.trim() ? ` DEFAULT ${parseDefault(f, db)}` : ""}${f.check && meta(f)?.hasCheck ? ` CHECK(${assertNoStatementBreak(f.check)})` : ""}`,
      )
      .join(",\n");
    ...
    fieldDefs = table.fields
      .map(
        (f) =>
          `\t${q(f.name)} ${f.type}${f.notNull ? " NOT NULL" : ""}${f.unique ? " UNIQUE" : ""}${f.default !== "" ? ` DEFAULT ${parseDefault(f, db)}` : ""}${f.check && meta(f)?.hasCheck ? ` CHECK(${assertNoStatementBreak(f.check)})` : ""}`,
      )
      .join(",\n");
  } else if (db === DB.MSSQL) {
    ...
    fieldDefs = table.fields
      .map((f) => {
        ...
        return `\t${q(f.name)} ${f.type}${f.size && isSized ? `(${f.size})` : ""}${f.notNull ? " NOT NULL" : ""}${f.increment ? " IDENTITY" : ""}${f.unique ? " UNIQUE" : ""}${f.default !== "" ? ` DEFAULT ${parseDefault(f, db)}` : ""}${f.check && meta(f)?.hasCheck ? ` CHECK(${assertNoStatementBreak(f.check)})` : ""}`;
      })
      .join(",\n");
    ...
    fieldDefs = table.fields
      .map(
        (f) =>
          `\t${q(f.name)} ${f.type}${f.size !== undefined && f.size !== "" ? `(${f.size})` : ""}${f.notNull ? " NOT NULL" : ""}${f.increment ? " GENERATED BY DEFAULT AS IDENTITY" : ""}${f.unique ? " UNIQUE" : ""}${f.default !== "" ? ` DEFAULT ${parseDefault(f, db)}` : ""}${f.check && meta(f)?.hasCheck ? ` CHECK(${assertNoStatementBreak(f.check)})` : ""}`,
      )
      .join(",\n");
  }
